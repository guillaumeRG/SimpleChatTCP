	...
	public void run() {
		while (isConnect) {
			...
			if (MsgType.equals("DATA")) {
				...
				String styleMembre = st.nextToken();
				String styleTexte = st.nextToken();
				...
				try {
					LauncherIhm.append(envoyeur, styleMembre, msg, styleTexte);
			...
			else if (MsgType.equals("INFO"))
			{
				...
				LauncherIhm.append("Information", "VERT", msg, "NOIR");
			}
			else if (MsgType.equals("KICKED"))
			{
				try {
					LauncherIhm.append("Kick par le serveur", "ROUGE");
				...
				LauncherIhm.connectChatIsAlive = false;
				try {
					soc.close();
				} catch (IOException e) {
					// TODO Auto-generated catch block
			...
			{
				try {
					LauncherIhm.append("Déconnecté du serveur", "ROUGE");
				} catch (BadLocationException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
