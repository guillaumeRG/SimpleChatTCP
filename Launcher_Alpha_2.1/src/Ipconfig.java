...
public class Ipconfig {
	...
	static InetAddress ipLocale = null;
	static InetAddress ipCible = null;
	static int port = 0;
	...
	public static String[] ouvrirFichierIpconfig() throws FileNotFoundException, UnknownHostException {
		...
		for (int i = 0; i < OuvrirFichier.compterLigne(ipconfigPath); i++) {
			...
			if(type.equals("ipLocale"))
			{
				ipLocale = InetAddress.getByName(contenu[i].split(" : ")[1]);
			}
			if(type.equals("ipCible"))
			{
				ipCible = InetAddress.getByName(contenu[i].split(" : ")[1]);
			}
			if(type.equals("port"))
			{
